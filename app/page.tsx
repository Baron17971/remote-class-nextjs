...
export default function App() {
  ...
  const generateFullPlanAI = async () => {
    ...
    const promptText = `אתה מומחה פדגוגי. בנה מערך שיעור סינכרוני מלא לפי מודל חז"ק.
      ...
      - עבור YouTube: אין להשתמש בקישור לסרטון ספציפי (למשל עם watch?v=). חובה להשתמש בקישור לתוצאות חיפוש בפורמט: https://www.youtube.com/results?search_query=[topic] (החלף את [topic] בנושא רלוונטי).
  ...
  const generateActivityDetailsAI = async (phaseId: string) => {
    ...
    const promptText = `אתה מומחה פדגוגי. המורה בחר את הפעילות הבאה לשלב "${phaseConfig.title}": "${activity}"
      ...
      - עבור YouTube: אין להשתמש בקישור לסרטון ספציפי (watch?v=). השתמש בקישור לחיפוש: https://www.youtube.com/results?search_query=[topic].
