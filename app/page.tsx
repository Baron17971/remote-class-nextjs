...
export default function App() {
  ...
  const generateFullPlanAI = async () => {
    ...
    const promptText = `אתה מומחה פדגוגי. בנה מערך שיעור סינכרוני מלא לפי מודל חז"ק.
      ...
      חובה קריטית על תקינות קישורים: חל איסור מוחלט לנחש או "להמציא" כתובות URL. כל קישור חייב להיות אמיתי ותקין. אם אינך בטוח בכתובת המדויקת של סרטון או עמוד פנימי, השתמש בכתובת הראשית של השירות (למשל: https://youtube.com, https://canva.com, https://padlet.com) והנחה את המורה מה לחפש שם. 
      אין צורך להוסיף רשימת כלים נפרדת בשדה "tools" - השאר אותו ריק או קצר מאוד, הקישורים חייבים להיות בגוף הטקסט בלבד.
      דוגמה: "יצירת אינפוגרפיקה ב-[Gemini](https://gemini.google.com) והקרנתה...".
  ...
  const generatePhaseAI = async (phaseId: string) => {
    ...
    const promptText = `אתה מומחה פדגוגי. הצע 3 אפשרויות שונות ומגוונות לשלב ה"${phaseConfig.title}" במערך שיעור סינכרוני.
      ...
      חובה קריטית על תקינות קישורים: חל איסור מוחלט לנחש או "להמציא" כתובות URL. כל קישור חייב להיות אמיתי ותקין. אם אינך בטוח בכתובת המדויקת, השתמש בכתובת הראשית של השירות (למשל: https://youtube.com, https://canva.com, https://padlet.com) והנחה את המורה מה לחפש שם. קישור שבור הוא תקלה חמורה.
  ...
  const generateActivityDetailsAI = async (phaseId: string) => {
    ...
    const promptText = `אתה מומחה פדגוגי. המורה בחר את הפעילות הבאה לשלב "${phaseConfig.title}": "${activity}"
      ...
      חל איסור מוחלט לנחש קישורים. במידה ואתה מוסיף קישור, ודא שהוא תקין ואמיתי בפורמט [טקסט](URL).
