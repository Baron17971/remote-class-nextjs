...
export default function App() {
  ...
  const generateFullPlanAI = async () => {
    ...
    const promptText = `אתה מומחה פדגוגי. בנה מערך שיעור סינכרוני מלא לפי מודל חז"ק.
      ...
      חובה עבור שלב ה"חריש": הצע פעילויות מעוררות סקרנות המבוססות על ויזואליה וכלים מתקדמים. תן עדיפות גבוהה לשימוש במחוללי בינה מלאכותית (Generative AI) כגון Gemini, NotebookLM או Canva AI ליצירת תוכן חכם (אינפוגרפיקות, חידות ויזואליות, סרטונים קצרים או עיצובים שמעוררים מחשבה).
      חובה: עבור כל שלב, הצע בדיוק 3 אפשרויות שונות. כל פעילות צריכה להיות קצרה וממוקדת (2-3 משפטים).
      חובה על קישורים: בכל פעם שאתה ממליץ על כלי דיגיטלי או סרטון (חפש סרטונים איכותיים ופופולריים), שלב אותו כקישור "חם" ישירות בתוך הטקסט של השדה "activity" בפורמט Markdown סטנדרטי: [שם הכלי/הסרטון](URL).
      חובה קריטית (מניעת קישורים שבורים - ה"אופציה הגרעינית"): חל איסור מוחלט על "ניחוש" או המצאת כתובות URL. 
      - עבור YouTube: חל איסור מוחלט להשתמש בקישורי watch?v= או youtu.be. חובה להשתמש אך ורק בקישור לתוצאות חיפוש בפורמט: https://www.youtube.com/results?search_query=[topic]. 
      - חוק ברזל: אם ציינת בטקסט יוצר או ערוץ (כמו Amoeba Sisters), שאילתת החיפוש חייבת לכלול אותו במפורש!
      - עבור כלים דיגיטליים: השתמש בכתובת הראשית בלבד (https://canva.com וכדומה).
      קישור שבור הוא כשל מערכתי חמור - ודא שכל [שם](URL) שאתה כותב יעבוד תמיד.
      אין פה שבלונה - הקישורים חייבים להיות חלק מרצף הטקסט של הפעילות. שדה ה-tools צריך להישאר ריק או קצר מאוד.
      דוגמה: "צפייה ב-[סרטונים על PCR של Amoeba Sisters ב-YouTube](https://www.youtube.com/results?search_query=Amoeba+Sisters+PCR+explanation) והסקת מסקנות...".
      ${advancedOptions.useAITools ? "חובה עליך לשלב בהצעות כלי פיצוח טכנולוגיים מתקדמים מאוד כגון Gemini, NotebookLM, מחוללי תמונות, Canva AI ופלטפורמות משחוק. תן עדיפות עליונה לכלים אלו ליצירת חידונים, תמונות, עיצובים וסרטונים." : "שלב רעיונות יצירתיים יחד עם כלים טכנולוגיים מתאימים (תעדוף ל-Canva AI, Gemini, NotebookLM)."}
      ${advancedOptions.useDifferential ? "חובה עליך לבנות כל הצעה כפעילות דיפרנציאלית: הצע פעילות בסיס, ולאחריה ציין בקצרה משפט של פתרון פיגום לתלמידים מתקשים ומשפט נוסף כאתגר הרחבה לתלמידים מתקדמים." : ""}
  ...
  const generatePhaseAI = async (phaseId: string) => {
    ...
    const promptText = `אתה מומחה פדגוגי. הצע 3 אפשרויות שונות ומגוונות לשלב ה"${phaseConfig.title}" במערך שיעור סינכרוני.
      ...
      ${contextAddition}
      ${phaseId === 'harish' ? 'דגש מיוחד לשלב ה"חריש": הצע פעילויות ויזואליות ומעוררות סקרנות. תן עדיפות גבוהה לכלים כגון Gemini, NotebookLM ו-Canva AI ליצירת תוכן חכם (אינפוגרפיקות, חידות, מצגות).' : ''}
      חובה על קישורים: שלב כל הצעה לכלי או סרטון (בחר סרטונים איכותיים ופופולריים) כקישור "חם" ישירות בתוך הטקסט של השדה "activity" בפורמט Markdown סטנדרטי: [שם הכלי/הסרטון](URL).
      חובה קריטית (מניעת קישורים שבורים - ה"אופציה הגרעינית"): חל איסור מוחלט על "ניחוש" או המצאת כתובות URL. 
      - עבור YouTube: חל איסור מוחלט להשתמש בקישורי watch?v= או youtu.be. חובה להשתמש אך ורק בקישור לתוצאות חיפוש בפורמט: https://www.youtube.com/results?search_query=[topic]. חובה לכלול בשאילתה את שם הערוץ/היוצר אם הוא מוזכר בטקסט (למשל: Amoeba+Sisters+PCR).
      - עבור כלים דיגיטליים: השתמש בכתובת הראשית של השירות בלבד (למשל: https://canva.com, https://padlet.com, https://kahoot.com).
      קישור שבור הוא כשל מערכתי חמור - ודא שכל [שם](URL) שאתה כותב יעבוד תמיד.
      אין פה שבלונה - הקישורים חייבים להיות חלק מרצף הטקסט של הפעילות. שדה ה-tools צריך להישאר ריק או קצר מאוד.
      ${advancedOptions.useAITools ? "חובה עליך לשלב בהצעות כלי פיצוח טכנולוגיים מתקדמים מאוד כגון Gemini, NotebookLM, מחוללי תמונות, Canva AI ופלטפורמות משחוק. תן עדיפות עליונה לכלים אלו." : "שלב רעיונות יצירתיים יחד עם כלים טכנולוגיים מתאימים (תעדוף ל-Canva AI, Gemini, NotebookLM)."}
      ${advancedOptions.useDifferential ? "חובה עליך לבנות כל הצעה כפעילות דיפרנציאלית: הצע פעילות בסיס, ולאחריה ציין בקצרה משפט של פתרון פיגום ומשפט נוסף כאתגר הרחבה." : ""}
  ...
  const generateActivityDetailsAI = async (phaseId: string) => {
    ...
    const promptText = `אתה מומחה פדגוגי. המורה בחר את הפעילות הבאה לשלב "${phaseConfig.title}": "${activity}"
      ...
      בנוסף, הוסף בתחתית ההסבר תחת הכותרת "**פרומפט בסיסי לביצוע:**" דוגמה לפרומפט (הנחיה) שהמורה או התלמיד יכולים להעתיק לכלי ה-AI (כמו Gemini או Canva) כדי לייצר את התוצר המבוקש.
      חובה קריטית (מניעת קישורים שבורים - ה"אופציה הגרעינית"): חל איסור מוחלט לנחש קישורים. במידה ואתה מוסיף קישור, ודא שהוא תקין ואמיתי בפורמט [טקסט](URL).
      - עבור YouTube: חל איסור מוחלט להשתמש בקישורי watch?v= או youtu.be. חובה להשתמש אך ורק בקישור לתוצאות חיפוש בפורמט: https://www.youtube.com/results?search_query=[topic]. חובה לכלול בשאילתה את שם הערוץ/היוצר אם הוא מוזכר בטקסט.
      - עבור כלים דיגיטליים: השתמש בכתובת הראשית של השירות בלבד (למשל: https://canva.com).
  ...
  const generateDifferentiationAI = async (phaseId: string) => {
    ...
    const promptText = `אתה מומחה פדגוגי. המורה בחר את הפעילות הבאה לשלב "${phaseConfig.title}": "${activity}"
      ...
      חובה על קישורים: אם אתה מוסיף קישור, חל איסור מוחלט לנחש כתובת ID של סרטון (watch?v=). השתמש רק בחיפוש ביוטיוב: https://www.youtube.com/results?search_query=[topic].
