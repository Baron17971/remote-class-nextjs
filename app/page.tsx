...
export default function App() {
  ...
  const generateFullPlanAI = async () => {
    ...
    const promptText = `אתה מומחה פדגוגי. בנה מערך שיעור סינכרוני מלא לפי מודל חז"ק.
      ...
      חובה על קישורים: בכל פעם שאתה ממליץ על כלי דיגיטלי או על סרטון (חפש סרטונים איכותיים ופופולריים), צרף קישור חי בפורמט Markdown סטנדרטי בלבד: [שם הכלי/הסרטון](URL). 
      דוגמה לפורמט תקין: [Gemini](https://gemini.google.com). אל תוסיף תווים מיותרים כמו פסיקים או סוגריים מחוץ לפורמט ה-Markdown. ודא שהקישורים תקינים.
  ...
  const generatePhaseAI = async (phaseId: string) => {
    ...
    const promptText = `אתה מומחה פדגוגי. הצע 3 אפשרויות שונות ומגוונות לשלב ה"${phaseConfig.title}" במערך שיעור סינכרוני.
      ...
      חובה על קישורים: בכל המלצה על כלי או סרטון (בחר סרטונים איכותיים עם הרבה צפיות), צרף קישור חי בפורמט Markdown סטנדרטי בלבד: [שם הכלי/הסרטון](URL). דוגמה: [Padlet](https://padlet.com). אל תוסיף תווים מיותרים.
                      ...
                      {(planData as any)[activeTab].suggestions.map((opt: any, idx: number) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="text-xs text-lilac-300 font-bold border-t border-white/5 pt-4 flex items-center gap-2 mt-auto">
                              <Pointer size={14} className="opacity-50" /> 
                              <MarkdownText text={opt.tools} />
                            </div>
                          </div>
                        );
                      })}
