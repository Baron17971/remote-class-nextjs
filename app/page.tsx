...
export default function App() {
  ...
  const generateFullPlanAI = async () => {
    ...
    const promptText = `אתה מומחה פדגוגי. בנה מערך שיעור סינכרוני מלא לפי מודל חז"ק.
      ...
      חובה על קישורים: שמופיע כלי דיגיטלי או סרטון (חפש סרטונים איכותיים ופופולריים), שלב אותו כקישור "חם" ישירות בתוך הטקסט של השדה "activity" בפורמט Markdown סטנדרטי: [שם הכלי/הסרטון](URL). 
  ...
  const generatePhaseAI = async (phaseId: string) => {
    ...
    const promptText = `אתה מומחה פדגוגי. הצע 3 אפשרויות שונות ומגוונות לשלב ה"${phaseConfig.title}" במערך שיעור סינכרוני.
      ...
      עבור כל אפשרות, כתוב תיאור פעילות פרקטי וברור של 2-3 משפטים בלבד.
      ...
      דוגמה: "התלמידים משתמשים ב-[Padlet](https://padlet.com) כדי לרכז...".
      אין צורך בכתיבה נפרדת של הכלים בשדה ה-tools, הכל חייב להיות חלק מרצף הטקסט של הפעילות.
