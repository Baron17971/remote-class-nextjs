...
export default function App() {
  ...
  const generateFullPlanAI = async () => {
    ...
    const promptText = `אתה מומחה פדגוגי. בנה מערך שיעור סינכרוני מלא לפי מודל חז"ק.
      ...
      חובה קריטית (מניעת קישורים שבורים): חל איסור מוחלט על "ניחוש" או המצאת כתובות URL. 
      - עבור YouTube: אין להשתמש בקישור לסרטון ספציפי (למשל עם watch?v=). חובה להשתמש בקישור לתוצאות חיפוש בפורמט: https://www.youtube.com/results?search_query=[topic]. חובה לכלול בשאילתה את שם הערוץ/היוצר אם הוא מוזכר בטקסט (למשל: Amoeba+Sisters+PCR).
      ...
      אין צורך להוסיף רשימת כלים נפרדת בשדה "tools" - השאר אותו ריק או קצר מאוד, הקישורים חייבים להיות בגוף הטקסט בלבד.
      דוגמה: "צפייה ב-[סרטונים על PCR ב-YouTube](https://www.youtube.com/results?search_query=pcr+explanation) והסקת מסקנות...".
  ...
  const generateActivityDetailsAI = async (phaseId: string) => {
    ...
    const promptText = `אתה מומחה פדגוגי. המורה בחר את הפעילות הבאה לשלב "${phaseConfig.title}": "${activity}"
      ...
      חובה קריטית (מניעת קישורים שבורים): חל איסור מוחלט לנחש קישורים. במידה ואתה מוסיף קישור, ודא שהוא תקין ואמיתי בפורמט [טקסט](URL).
      - עבור YouTube: אין להשתמש בקישור לסרטון ספציפי (watch?v=). השתמש בקישור לחיפוש רלוונטי: https://www.youtube.com/results?search_query=[topic] (הקפד לכלול שם ערוץ/יוצר אם הוא מוזכר בטקסט).
